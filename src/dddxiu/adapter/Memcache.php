<?php

namespace Dddxiu\adapter;

/**
 * Memcache缓存
 */
class Memcache implements AdapterImp
{
    /**
     * 设置值
     * @param [type] $key       [description]
     * @param [type] $value     [description]
     * @param [type] $expire_at [description]
     */
    public function put(string $key, $value, $expire_at)
    {
        return false;
    }


    /**
     * 批量取值
     * @param  array  $kvs       [description]
     * @param  [type] $expire_at [description]
     * @return [type]            [description]
     */
    public function puts(array $kvs, $expire_at)
    {
        return false;
    }


    /**
     * 取值
     * @param  [type] $key     [description]
     * @param  [type] $default [description]
     * @return [type]          [description]
     */
    public function get(string $key, $default=NULL)
    {
        return false;
    }


    /**
     * 批量取值
     * @param  [type] $key [description]
     * @return [type]      [description]
     */
    public function gets(string $key)
    {
        return false;
    }


    /**
     * 自增
     * @param  string  $key  [description]
     * @param  integer $step [description]
     * @return [type]        [description]
     */
    public function inc(string $key, $step=1)
    {
        return false;
    }


    /**
     * 减少
     * @param  string  $key  [description]
     * @param  integer $step [description]
     * @return [type]        [description]
     */
    public function dec(string $key, $step=1)
    {
        return false;
    }


    /**
     * 删除一个值
     * @param  string $key [description]
     * @return [type]      [description]
     */
    public function del(string $key)
    {
        return false;
    }


    /**
     * 亲空缓存
     * @return [type] [description]
     */
    public function flush()
    {
        return false;
    }


    public function load(array $conf)
    {
        var_dump($conf);
    }
}
